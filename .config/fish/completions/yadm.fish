...
# filter unwanted git commands (because they would take too much time, like add)
function __fish_yadm_can_git_complete
  __fish_yadm_using_command add 
  and return 1

  return 0
end

# -w does not work in conjonction of -n thus, we fake it
function __fish_yadm_git_wrap
  # wraps git's autocomplete
  set -l GIT_DIR (yadm introspect repo)

  set -l cmd (commandline -pc | string split -m 1 " ") 
  set -e cmd[1]
  complete -C "git --git-dir=$GIT_DIR $cmd"
end


# setup the correct git-dir by appending it to git's argunment
complete -c yadm -n "__fish_yadm_can_git_complete" -a "(__fish_yadm_git_wrap)" 

function __fish_yadm_git_add
  # for know, there is no . completion, this will change in 3.7.0
  # (https://github.com/fish-shell/fish-shell/issues/3707)
  yadm status --porcelain=v1 | while read -l -a line
    echo -e "$line[2]\t$line[1]"
  end
end
complete -c yadm -n "__fish_yadm_using_command add" -a "(__fish_yadm_git_add)" 
