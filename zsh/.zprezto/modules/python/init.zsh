...
if (( $#commands[(i)pip(|[23])] )); then
    ...
    cache_file="/tmp/prezto-python-cache.$UID.zsh"
    pip_completion_worker(){
    ...
    pip_completion_callback(){
        source "$cache_file"
        unset cache_file pip_command
    }

    async_start_worker pip_worker
    async_register_callback pip_worker pip_completion_callback
    async_job pip_worker pip_completion_worker
fi
